...
export default function ToastContainer() {
  ...
  return <ToastInner isVisible={isVisible} state={state} />;
}
...
function ToastInner({
  ...
  // 获取 portal 容器,如果不存在则创建
  let portalContainer = document.getElementById('toast-ui');
  if (!portalContainer) {
    portalContainer = document.createElement('div');
    portalContainer.id = 'toast-ui';
  ...
  return createPortal(
    <>
      {renderState.mask && (
        <div
          className={cn(
            'fixed inset-0 z-[9999] bg-transparent transition-opacity duration-300',
      ...
      <div
        className={cn(
          'fixed z-[10000] min-w-[120px] flex flex-col justify-center items-center space-y-2 rounded-lg dark:bg-gray-700/95 bg-black/80 p-4 text-white duration-300',
